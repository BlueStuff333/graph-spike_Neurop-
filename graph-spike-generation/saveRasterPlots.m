function saveRasterPlots(firings, node_colors, raster_dir, id)
  ...
  ylabel('Neuron Index');
  title('Raster');
  filename = sprintf('raster_%d.jpg', id);
  exportgraphics(fig1, fullfile(raster_dir, filename), 'Resolution', 300);
  close(fig1);
  ...
  title('Raster');
  filename = sprintf('raster_clustered_%d.jpg', id);
  exportgraphics(fig2, fullfile(raster_dir, filename), 'Resolution', 300);
  close(fig2);
