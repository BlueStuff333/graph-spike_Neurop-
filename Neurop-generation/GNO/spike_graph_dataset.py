...
class SpikeGraphDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        events[:, 1] -= 1  # convert to 0-based neuron indices from 1-based MATLAB
        adj = np.asarray(adj)
...
def spike_graph_collate(batch: list) -> dict:
    ...
    if params_list:
        lengths = torch.tensor(
            [p.numel() for p in params_list],
            dtype=torch.long,
        )
        max_len = lengths.max().item()

        padded = torch.zeros(len(params_list), max_len, dtype=torch.float32)
        for i, p in enumerate(params_list):
            padded[i, : p.numel()] = p

        out["mf_params"] = padded          # [B, max_len]
        out["mf_params_len"] = lengths     # [B]
